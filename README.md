...
- **Scratch memory**: A shared scratch log with session IDs, timestamps, and optional Thread tags. Multiple sessions can read and write to it concurrently; passing `thread_id` scopes the log to the Thread you're working in, so parallel work on another project doesn't bleed into your context. Untagged entries stay visible from every Thread, which is what you want for Dream summaries and threadless chat. It gets compacted at session end, and key insights are promoted to long-term memory storage.
...
**Note**: Each new Engram session bootstraps by calling `soul(action: "get")`, then `thread(action: "resolve")` (auto-detects or creates the active Thread from the working directory), then `context(action: "load")`, then `scratch(action: "read", bootstrap: true, thread_id)` to gather the relevant starting context for the session. The `thread_id` from `resolve` is passed to both `context` and `scratch` so the session stays scoped to one Thread.
