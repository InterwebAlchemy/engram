...
For Claude Code, optionally generate .claude/CLAUDE.md with `npm run sync:agents`. -->
...
- `AGENTS.md` is the repo-level contributor guide (read by most AI coding tools)
- For Claude Code, optionally generate `.claude/CLAUDE.md` via `npm run sync:agents` (gitignored, opt-in)
